...
almost every number is the sum of two of the previous 5 numbers; the only number that does not follow this rule is 127.
...
class Solve:
    ...
    @property
    def preamble(self):
        return self._preamble_work

    @preamble.setter
    def preamble(self, start_index):
        if start_index + self.PREAMBLE_LEN <= len(self._data):
            end_index = start_index + self.PREAMBLE_LEN
            self._preamble_work = self._data[start_index:end_index]
        else:
            self._limit_reached = True

    @property
    def results(self):
        return self.iterate_on_data()

    @property
    def errors(self):
        return {k: v for k, v in self.results.items() if v is False}

    def parse_data_from_lines(self, lines):
        parsed = [int(l.strip()) for l in lines]

        # Temporarily stopping parsing before invalid number (127)
        # temp_parsed = parsed[:14]
        temp_parsed = parsed

        return temp_parsed

    def can_sum_preamble(self, preamble, data):
        for p in preamble:
            for o in preamble:
                if o + p == data:
                    return True
    ...
    def can_sum_contiguous_range(self, max_index, data):
        d_keys = self._data
        for i in range(max_index):
            for j in range(max_index):
                if int(sum(d_keys[i:j])) == int(data):
                    return True, range(i, j)
        return

    def iterate_on_data(self):
        result_dict = {}

        for i in range(self.PREAMBLE_LEN, len(self._data)):
            d = self._data[i]
            self.preamble = i - self.PREAMBLE_LEN
            result = self.can_sum_preamble(self.preamble, d)
            result_dict[str(d)] = result
    ...
    def contiguous_check(self):
        for e in self.errors:
            index_of_error = list(self.results.keys()).index(e)
            can_sum, range_ = self.can_sum_contiguous_range(index_of_error, e)
            if can_sum:
                start = range_[0]
                end = range_[-1] + 1
                min_ = min(self._data[start:end])
                max_ = max(self._data[start:end])
                return min_ + max_
        return 1
